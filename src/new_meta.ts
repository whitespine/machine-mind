import { Action, MechWeapon } from '@/class';
...
import { IActionData, ICoreBonusData, IDeployableData, IEnvironmentData, IFactionData, IFrameData, IManufacturerData, IMechSystemData, IMechWeaponData, INpcClassData, INpcFeatureData, INpcTemplateData, IPilotArmorData, IPilotData, IPilotEquipmentData, IPilotGearData, IPilotWeaponData, IReserveData, ISitrepData, ISkillData, IStatusData, ITalentData, IWeaponModData } from './interface';
...
export abstract class SerUtil {
    ...
    public static restrict_choices<T extends string>(choices: T[], default_choice: T, provided?: string): T {
        return choices.includes(provided as T) ? (provided as T) : default_choice;
    }

    public static restrict_enum<T extends string>(enum_: {[key: string]: T}, default_choice: T, provided: string): T {
...
export abstract class RegSer<T extends EntryType, SourceType> {
    public readonly Type: T;
    public readonly Registry: Registry;
    private _load_promise: Promise<any>;

    // Setup
    constructor(type: T, registry: Registry, data: SourceType) {
        this.Type = type;
...
export abstract class RegEntry<T extends EntryType, SourceType> {
    ...
    // Setup
    constructor(type: T, registry: Registry, reg_id: string | typeof CREATE_ENTRY, fallback_init: SourceType) {
        this.type = type;
        this.Registry = registry;
        ...
        if(typeof reg_id === "string") {
            this.RegistryID = reg_id;
            this._load_promise = registry.get_cat(type);
        } else {
            // Do as he says
            this.RegistryID = "";
        }
...
export abstract class RegCat<T extends EntryType> {
    ...
    constructor(parent: Registry, cat: T, creator: CreationFunc<RegEntryTypes[T], LiveEntryTypes[T]>) {
        this.parent = parent;
        this.cat = cat;
    ...
    abstract async create(...vals: Array<RegSer<T, RegEntryTypes[T]>>): Promise<LiveEntryTypes[T][]>;
...
export class Registry {
    ...
    async get_from_anywhere(id: string): Promise<RegSer<any, any> | null> {
